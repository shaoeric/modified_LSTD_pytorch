    ...
    def predict(frame):
        ...
        for i in range(detections.size(0)):
            for r in range(detections.size(2)):
                ...
                if max_score.data >= 0.05:
                    pt = (detections[i, idx, r, 1:] * scale).cpu().numpy()
                    cv2.rectangle(frame,
                                  (int(pt[0]), int(pt[1])),
                                  (int(pt[2]), int(pt[3])),
                                  COLORS[r % 3], 2)
                    cv2.putText(frame, voc_label[idx - 1], (int(pt[0]), int(pt[1]+20)), FONT, 0.5, (0, 255, 255), 2, cv2.LINE_AA)
        return frame

    img = cv2.imread("E:\python_project\ssd\ssdpytorch\dataset\VOC\VOCdevkit\VOC2007\JPEGImages\\000051.jpg")
...
net = build_source('detect', 300).cuda()
