...
class MultiBoxLoss(nn.Module):
    ...
    def forward(self, predictions, targets):
        ...
        batch_conf = conf_data.view(-1, self.num_classes) # [batch, 8732, 2]
        loss_c = log_sum_exp(batch_conf) - batch_conf.gather(1, conf_t.view(-1, 1))
        # print(batch_conf.gather(1, conf_t.view(-1, 1)).min(), batch_conf.gather(1, conf_t.view(-1, 1)).max())
        # t = float(log_sum_exp(batch_conf).min().data.cpu().numpy())
        # if t == float('-inf'):
        #     print('inf',batch_conf, batch_conf.min(), batch_conf.max())
