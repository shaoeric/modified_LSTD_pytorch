...
class ToCV2Image(object):
    def __call__(self, tensor, boxes=None, labels=None, mask=None):
        return tensor.cpu().numpy().astype(np.float32).transpose((1, 2, 0)), boxes, labels, mask


class ToTensor(object):
    def __call__(self, cvimage, boxes=None, labels=None, mask=None):
        return torch.from_numpy(cvimage.astype(np.float32)).permute(2, 0, 1), boxes, labels, mask
...
class RandomSampleCrop(object):
    ...
    def __call__(self, image, boxes=None, labels=None, mmask=None):
        height, width, _ = image.shape
        while True:
            # randomly choose a mode
            mode = random.choice(self.sample_options)
            if mode is None:
                return image, boxes, labels, mmask
...
class Expand(object):
    ...
    def __call__(self, image, boxes, labels, mask=None):
        ...
        if mask is not None:
            ...
            expand_mask[int(top):int(top + height), int(left):int(left + width)] = mask
            mask = expand_mask
...
class RandomMirror(object):
    def __call__(self, image, boxes, classes, mask=None):
...
class SSDAugmentation(object):
    ...
    def __call__(self, img, boxes, labels, mask):
        return self.augment(img, boxes, labels, mask)
